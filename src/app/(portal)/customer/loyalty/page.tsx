              ...
              {ledger.map((entry) => {
                ...
                return (
                  <li key={entry.id} className="flex flex-col justify-between gap-4 bg-paper p-5 transition hover:bg-cream/35 sm:flex-row sm:items-center">
                    <div className="flex items-center gap-4"><span className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl ${positive ? "bg-leaf/10 text-leaf" : "bg-coral/10 text-coral"}`}>{positive ? <ArrowUpRight className="h-5 w-5" /> : <ArrowDownRight className="h-5 w-5" />}</span><div><p className="font-bold">{entry.reason}</p><p className="mt-1 text-xs text-ink/80">{entry.createdAt.toLocaleString("en-IN", { dateStyle: "medium", timeStyle: "short" })}</p></div></div>
                    <div className="pl-15 sm:pl-0 sm:text-right"><p className={`font-display text-2xl font-semibold ${positive ? "text-leaf" : "text-coral"}`}>{positive ? "+" : "−"}{money(Math.abs(entry.delta))}</p><p className="mt-1 text-xs text-ink/80">Balance {money(entry.balanceAfter)}</p></div>
                  </li>
                );
              })}
            </ul>
