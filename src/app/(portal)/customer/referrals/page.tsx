    ...
    <PortalShell mode="customer" displayName={identity.displayName} showSummaryCards={false}>
      ...
      <div className="mt-5 grid gap-5 xl:grid-cols-[0.82fr_1.18fr]">
        ...
        <DashboardPanel>
          ...
          {referrals.length ? <div className="mt-7 grid gap-3">{referrals.map((referral) => <article key={referral.id} className="rounded-[1.6rem] border border-ink/[0.07] bg-cream/45 p-5 transition hover:border-indigo/20 hover:bg-paper hover:shadow-lifted"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-[0.6rem] font-bold uppercase tracking-[0.14em] text-coral">Referral code</p><p className="mt-1 font-mono text-sm font-bold tracking-[0.12em] text-indigo">{referral.code}</p></div><StatusPill status={referral.status} /></div><h3 className="mt-5 font-display text-2xl font-semibold tracking-[-0.035em]">{referral.referred?.displayName ?? "Invitation waiting"}</h3><div className="mt-4 grid grid-cols-3 gap-2"><ReferralState active icon={HeartHandshake} label="Shared" /><ReferralState active={Boolean(referral.qualifiedAt)} icon={UserRoundCheck} label="Qualified" /><ReferralState active={Boolean(referral.rewardedAt)} icon={BadgeCheck} label="Rewarded" /></div><p className="mt-4 text-xs text-ink/80">Created {referral.createdAt.toLocaleDateString("en-IN", { dateStyle: "medium" })}{referral.rewardedAt ? ` · rewarded ${referral.rewardedAt.toLocaleDateString("en-IN", { dateStyle: "medium" })}` : ""}</p></article>)}</div> : <div className="mt-7"><DashboardEmptyState icon={HeartHandshake} title="No invitations yet." description="An authorised referral campaign will place a shareable code here. There are no hidden or provisional rewards." /></div>}
        </DashboardPanel>
      </div>
    </PortalShell>
  );
