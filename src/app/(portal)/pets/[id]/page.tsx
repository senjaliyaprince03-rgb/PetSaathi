      ...
      <div className="mt-5">
        ...
        <section className="luxury-grid relative mt-5 overflow-hidden rounded-5xl border border-indigo/10 bg-gradient-to-br from-paper via-[#f3eafa] to-[#fff0e8] p-7 shadow-soft sm:p-10">
          ...
          <div className="relative flex flex-col gap-7 sm:flex-row sm:items-center">
            ...
            <div><p className="eyebrow">Digital pet passport</p><h1 className="mt-4 font-display text-4xl sm:text-6xl font-semibold leading-none tracking-[-0.055em]">{pet.name}</h1><p className="mt-3 text-ink/80">{pet.species.toLowerCase()}{pet.breed ? ` · ${pet.breed}` : ""}{pet.weightKg ? ` · ${pet.weightKg.toString()} kg` : ""}</p><div className="mt-4 flex flex-wrap gap-2"><span className="rounded-full bg-leaf/10 px-3 py-1.5 text-xs font-bold text-leaf">Private record</span><span className="rounded-full bg-indigo/10 px-3 py-1.5 text-xs font-bold text-indigo">ID · {pet.id.slice(0, 8).toUpperCase()}</span></div><Link href={`/pets/${pet.id}/id-card` as Route} className={`${buttonVariants({ variant: "outline", size: "sm" })} mt-5`}><CreditCard className="h-4 w-4" />Open digital ID card</Link></div>
          </div>
        </section>
        ...
        <div className="mt-6 grid gap-5 xl:grid-cols-2">
          <section className="rounded-4xl border border-indigo/10 bg-paper p-6 shadow-lifted">
            ...
            <div className="mt-5 grid gap-3">
              {pet.medications.map((item) => <article key={item.id} className="rounded-2xl bg-cream/45 p-4"><div className="flex items-center justify-between gap-3"><p className="flex items-center gap-2 font-semibold"><Pill className="h-4 w-4 text-coral" />{item.name}</p><span className={`rounded-full px-3 py-1 text-xs font-bold ${item.active ? "bg-leaf/10 text-leaf" : "bg-ink/5 text-ink/80"}`}>{item.active ? "ACTIVE" : "ENDED"}</span></div><p className="mt-2 text-sm text-ink/80">{item.dosage} · {item.schedule}</p>{item.administration && <p className="mt-1 text-sm text-ink/80">Admin: {item.administration}</p>}{item.prescribedBy && <p className="mt-1 text-sm text-ink/80">Prescribed by: {item.prescribedBy}</p>}{item.notes && <p className="mt-1 text-sm text-ink/80">Notes: {item.notes}</p>}{item.startsAt && <p className="mt-1 text-xs text-ink/80">From: {item.startsAt.toLocaleDateString("en-IN")}{item.endsAt ? ` to ${item.endsAt.toLocaleDateString("en-IN")}` : ""}</p>}{item.active && <MedicationStatusButton petId={pet.id} medicationId={item.id} />}</article>)}
              {pet.vaccinations.map((item) => <article key={item.id} className="rounded-2xl bg-indigo/5 p-4"><p className="flex items-center gap-2 font-semibold"><ShieldCheck className="h-4 w-4 text-indigo" />{item.vaccine}</p><p className="mt-2 text-sm text-ink/80">Given {item.administeredAt.toLocaleDateString("en-IN")}{item.nextDueAt ? ` · due ${item.nextDueAt.toLocaleDateString("en-IN")}` : ""}</p>{item.clinic && <p className="mt-1 text-sm text-ink/80">Clinic: {item.clinic}</p>}</article>)}
              {!pet.medications.length && !pet.vaccinations.length && <p className="text-sm text-ink/80">No structured medication or vaccination records.</p>}
            </div>
          </section>
          <section className="rounded-4xl border border-indigo/10 bg-paper p-6 shadow-lifted">
            <h2 className="font-display text-3xl font-semibold">Health timeline</h2>
            <div className="mt-5 grid gap-3">{pet.healthEvents.length ? pet.healthEvents.map((event) => <article key={event.id} className="rounded-2xl border-l-4 border-saffron bg-cream/40 p-4"><p className="text-xs font-bold uppercase tracking-[0.14em] text-coral">{event.eventType.replaceAll("_", " ")}</p><p className="mt-2 font-semibold">{event.summary}</p><p className="mt-2 flex items-center gap-2 text-xs text-ink/80"><CalendarDays className="h-3.5 w-3.5" />{event.occurredAt.toLocaleString("en-IN")}</p></article>) : <p className="text-sm text-ink/80">No timeline events recorded.</p>}</div>
          </section>
        </div>
        <PetHealthRecordForms petId={pet.id} />
        <p className="mt-5 rounded-3xl border border-saffron/20 bg-saffron/8 p-5 text-sm leading-6 text-ink/80"><strong>Medical disclaimer:</strong> These records organise information supplied by the pet parent. They do not replace veterinary advice, diagnosis, prescriptions or emergency treatment.</p>
      </div>
    </PortalShell>
  );
}
