import { BadgeCheck, HeartPulse, PawPrint, ShieldCheck } from "lucide-react";
...
export default async function DigitalPetIdPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const pet = await prisma.pet.findFirst({
    ...
    include: {
      ...
      _count: { select: { vaccinations: true, medications: true, healthEvents: true } }
    }
  });

  if (!pet) notFound();

  const code = pet.id.replaceAll("-", "").slice(0, 16).toUpperCase();

  return (
    <PortalShell
      mode="customer"
      displayName={identity.displayName}
      metrics={[`${pet._count.vaccinations} vaccinations`, `${pet._count.medications} medications`, `${pet._count.healthEvents} health events`]}
    >
      <div className="mx-auto mt-5 max-w-4xl">
        <section className="relative overflow-hidden rounded-[3rem] bg-[#2f2032] p-7 text-paper shadow-soft sm:p-10">
          <div className="absolute -right-16 -top-20 h-72 w-72 rounded-full bg-coral/25 blur-3xl" />
          <div className="relative flex flex-col justify-between gap-10 sm:flex-row">
            <div>
              ...
              <div className="mt-7 flex flex-wrap gap-2">
                <span className="flex items-center gap-2 rounded-full bg-paper/10 px-3 py-2 text-xs font-bold">
                  <ShieldCheck className="h-4 w-4 text-saffron" />
                  Private owner record
                </span>
                <span className="flex items-center gap-2 rounded-full bg-paper/10 px-3 py-2 text-xs font-bold">
                  <BadgeCheck className="h-4 w-4 text-saffron" />
                  Active profile
                </span>
              </div>
            </div>
            <span className="flex h-32 w-32 shrink-0 items-center justify-center rounded-[2.3rem] border border-paper/15 bg-paper/10">
              <PawPrint className="h-14 w-14 text-saffron" strokeWidth={1.4} />
            </span>
          </div>
