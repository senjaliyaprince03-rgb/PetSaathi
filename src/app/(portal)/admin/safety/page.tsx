...
export default async function AdminSafetyPage() {
  ...
  return <main className="min-h-screen bg-cream/50 py-10"><div className="container-shell"><p className="eyebrow">purpose-limited access</p><h1 className="section-title mt-5">Safety command queue</h1><p className="mt-5 max-w-3xl text-lg leading-8 text-ink/60">Every incident stays open through triage, response, owner communication, review and any required corrective action. Booking recovery and Saathi holds are explicit, reasoned decisions.</p>{!canAct && <p className="mt-5 rounded-2xl bg-saffron/15 p-4 text-sm font-semibold">Operations has read-only incident visibility. Safety or Super Admin authority is required for timeline decisions, holds and closure.</p>}<div className="mt-10 grid gap-6">{incidents.length ? incidents.map((incident) => {
