...
export default async function CityHealthPage({ params }: PageProps) {
  ...
  // Check roles (SUPER_ADMIN, OPERATIONS_ADMIN, CITY_MANAGER)
  const isAuthorized =
  ...
  if (!isAuthorized) {
    return (
      <div className="container-shell p-8">
        ...
        <p className="text-ink/60">You do not have permission to view city health metrics.</p>
      </div>
    );
  }

  // Fetch the city and its records
    ...
    <div className="container-shell p-6 md:p-10 space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
        <div>
          ...
          <h1 className="section-title font-display mt-2">{city.name} Health</h1>
          <p className="text-ink/60 mt-2">
            Status: <span className="font-medium text-ink">{city.status}</span>
          </p>
        </div>
