            ...
            <table className="w-full text-left">
              <thead>
                <tr className="border-b border-indigo/10 bg-cream/30">
                  <th className="px-6 py-4 text-xs font-bold uppercase tracking-[0.16em] text-ink/80">Programme Name</th>
                  <th className="px-6 py-4 text-xs font-bold uppercase tracking-[0.16em] text-ink/80">Type / Method</th>
                  <th className="px-6 py-4 text-xs font-bold uppercase tracking-[0.16em] text-ink/80">Status</th>
                  <th className="px-6 py-4 text-xs font-bold uppercase tracking-[0.16em] text-ink/80">Members</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-indigo/5">
                  ...
                  <tr key={prog.id} className="transition-colors hover:bg-cream/20">
                    <td className="px-6 py-5">
                      ...
                      <div className="mt-1 flex items-center gap-2">
                        <span className="text-xs font-bold text-ink/80">/{prog.slug}</span>
                        <span className="text-xs text-ink/80">•</span>
                        <span className="text-xs text-ink/80">{prog.organization.displayName}</span>
                      </div>
                    </td>
                    <td className="px-6 py-5">
                      <div className="text-sm font-medium text-ink/80">{prog.programmeType.replace(/_/g, " ")}</div>
                      <div className="mt-1 text-xs text-ink/80">{prog.eligibilityMethod.replace(/_/g, " ")}</div>
                    </td>
                    <td className="px-6 py-5">
                      <span className={`inline-flex rounded-full px-3 py-1 text-xs font-bold ${
                        prog.status === "ACTIVE_PROGRAMME" ? "bg-leaf/10 text-leaf" : "bg-ink/10 text-ink/80"
                      }`}>
                ...
                {result.items.length === 0 && (
                  <tr>
                    <td colSpan={4} className="px-6 py-12 text-center text-sm font-medium text-ink/80">
                      No programmes found.
                    </td>
                  </tr>
                )}
