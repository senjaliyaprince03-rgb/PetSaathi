...
function CareProtocolGuide() {
  ...
  return <aside className="relative overflow-hidden rounded-[1.75rem] bg-[#281d2b] p-6 text-paper"><div className="absolute -right-16 -top-16 h-52 w-52 rounded-full bg-coral/20 blur-3xl" /><div className="relative"><span className="flex h-11 w-11 items-center justify-center rounded-2xl bg-saffron text-ink"><CalendarDays className="h-5 w-5" /></span><p className="mt-7 text-[0.6rem] font-bold uppercase tracking-[0.2em] text-saffron">Four protected gates</p><h2 className="mt-3 font-display text-3xl font-semibold tracking-[-0.04em]">A request, not a guessing game.</h2><div className="relative mt-7 grid gap-5 before:absolute before:bottom-4 before:left-4 before:top-4 before:w-px before:bg-paper/10">{steps.map(([number, title, copy]) => <article key={number} className="relative flex gap-4"><span className="z-10 flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-paper text-[0.62rem] font-bold text-ink">{number}</span><div><h3 className="text-sm font-bold">{title}</h3><p className="mt-1 text-xs leading-5 text-paper/80">{copy}</p></div></article>)}</div><p className="mt-7 rounded-2xl border border-paper/10 bg-paper/[0.06] p-4 text-xs leading-5 text-paper/80"><BadgeCheck className="mb-2 h-4 w-4 text-saffron" />Payment begins only after you approve a verified Saathi proposal.</p></div></aside>;
}
