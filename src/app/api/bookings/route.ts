...
export async function GET() {
  ...
  if (!user || (user.role !== "CUSTOMER" && user.role !== "SUPER_ADMIN")) {
    return problem(401, "unauthorized", "Sign in as a pet parent to view bookings.");
  }
...
export async function POST(request: Request) {
  ...
  if (!user || (user.role !== "CUSTOMER" && user.role !== "SUPER_ADMIN")) {
    return problem(401, "unauthorized", "Sign in as a pet parent to create a booking.");
  }
  ...
  const rate = await consumeRateLimit("booking-create-user", user.id, 10, 60 * 60_000);
  ...
  try {
    const booking = await createBookingWithQuote(user.id, parsed.data);
    ...
    logger.exception("booking.create_failed", error as Error, {
      customerId: user.id,
    });
