...
type ProviderMessage = { channel: NotificationChannel; destination: string; templateKey: string; payload: unknown };

export async function sendProviderMessage(message: ProviderMessage) {
  if (message.channel === "IN_APP") return { providerMessageId: `in-app:${randomUUID()}`, providerPayload: { accepted: true } };
...
async function sendEmail(message: ProviderMessage) {
  ...
  const { data, error } = await resend.emails.send({ from, to: message.destination, subject: rendered.subject, text: rendered.text });
