...
export function BookingWizard({ 
  ...
  return (
    <form onSubmit={form.handleSubmit(onValidSubmit, () => setAttemptedStep(3))} className="glass-panel mx-auto max-w-3xl rounded-5xl p-5 sm:p-9" noValidate>
      ...
      <fieldset className={cn(step !== 1 && "hidden")}><legend className="font-display text-3xl font-semibold">Who are we caring for?</legend><div className="mt-7 grid gap-5 sm:grid-cols-2"><Field label="Pet name" error={errorFor("petName")}><input {...form.register("petName")} maxLength={50} className="form-input" placeholder="e.g. Miso" /></Field><Field label="Pet type" error={errorFor("petType")}><select {...form.register("petType")} className="form-input"><option value="DOG">Dog</option><option value="CAT">Cat</option><option value="RABBIT">Rabbit</option><option value="BIRD">Bird</option><option value="FISH">Fish</option><option value="TURTLE">Turtle</option><option value="RAT">Rat / Mouse</option><option value="OTHER">Other</option></select></Field></div></fieldset>

      <fieldset className={cn(step !== 2 && "hidden")}><legend className="font-display text-3xl font-semibold">When and where?</legend><div className="mt-7 grid gap-5 sm:grid-cols-2"><Field label="Date" icon={<CalendarDays className="h-4 w-4" />} error={errorFor("date")}><input type="date" min={todayMinDate} {...form.register("date")} className="form-input" /></Field><Field label="Start time" icon={<Clock3 className="h-4 w-4" />} error={errorFor("time")}><input type="time" min="06:00" max="21:00" {...form.register("time")} className="form-input" /></Field><div className="sm:col-span-2"><Field label="Locality" icon={<MapPin className="h-4 w-4" />} error={errorFor("locality")}><input {...form.register("locality")} maxLength={100} className="form-input" placeholder="Bopal, Ahmedabad" /></Field></div></div></fieldset>

      <fieldset className={cn(step !== 3 && "hidden")}><legend className="font-display text-3xl font-semibold">How can we reach you?</legend><div className="mt-7 grid gap-5 sm:grid-cols-2"><Field label="Your name" error={errorFor("parentName")}><input {...form.register("parentName")} maxLength={100} className="form-input" autoComplete="name" /></Field><Field label="Mobile number" error={errorFor("phone")}><input {...form.register("phone")} className="form-input" inputMode="numeric" autoComplete="tel" placeholder="10-digit mobile" /></Field><div className="sm:col-span-2"><Field label="Anything useful for the first call?" error={errorFor("notes")}><textarea {...form.register("notes")} maxLength={800} className="form-input min-h-28 resize-y" placeholder="Routine, temperament, access or timing notes" /></Field></div></div></fieldset>
