...
const reportEvents = [
  { time: "09:02", label: "Care started", detail: "Saathi checked in within the approved service window.", icon: CheckCircle2 },
  { time: "09:16", label: "Routine complete", detail: "Food, water and care instructions were recorded.", icon: HeartPulse },
  { time: "09:38", label: "Photo update", detail: "A private update was attached to the booking timeline.", icon: Camera },
  { time: "10:00", label: "Handover ready", detail: "Service summary and exceptions were prepared for review.", icon: MessageCircleMore }
] as const;
                ...
                <AnimosCard className="h-full" glare={false}>
                  <article className="group flex h-full flex-col overflow-hidden rounded-5xl border border-indigo/10 bg-paper">
                    <div className="relative aspect-[4/3] overflow-hidden">
                      ...
                      <span className="absolute left-4 top-4 inline-flex items-center gap-1.5 rounded-full border border-paper/35 bg-paper/90 px-3 py-2 text-[0.62rem] font-bold text-leaf backdrop-blur-xl">
                        <BadgeCheck className="h-3.5 w-3.5" /> Example proposal
                      </span>
                      <span className="absolute bottom-4 left-4 rounded-full border border-paper/20 bg-ink/42 px-3 py-2 text-[0.62rem] font-bold text-paper backdrop-blur-xl">
                    ...
                    <div className="grid gap-3">
                      {reportEvents.map(({ time, label, detail, icon: Icon }, index) => (
                        <div key={label} className="relative grid grid-cols-[3.4rem_2.6rem_1fr] items-start gap-3">
                          {index < reportEvents.length - 1 ? <span className="absolute left-[4.65rem] top-9 h-[calc(100%+0.75rem)] w-px bg-indigo/[0.12]" aria-hidden="true" /> : null}
                          <time className="pt-2 text-[0.65rem] font-bold text-ink/38">{time}</time>
                          <span className="relative z-10 flex h-10 w-10 items-center justify-center rounded-2xl bg-indigo/[0.07] text-indigo">
