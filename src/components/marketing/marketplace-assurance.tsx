...
export function MarketplaceAssurance() {
  return (
    <>
      <section className="py-12 sm:py-16" aria-labelledby="proposal-preview-title">
        <div className="container-shell">
          ...
          <ScrollStaggerContainer className="mt-12 grid gap-5 lg:grid-cols-3">
            {proposalPreviews.map((proposal, index) => (
              <ScrollStaggerItem key={proposal.service}>
                <article className="group relative flex h-full flex-col overflow-hidden rounded-[2.5rem] border border-indigo/10 bg-paper shadow-sm transition-all duration-500 hover:-translate-y-2 hover:shadow-2xl hover:border-indigo/20">
                  <div className="relative aspect-[4/3] overflow-hidden">
                    ...
                    <div className="absolute left-5 top-5">
                      <span className="inline-flex items-center gap-1.5 rounded-full bg-paper/95 px-3.5 py-2 text-[0.65rem] font-bold uppercase tracking-widest text-leaf shadow-sm">
                        <BadgeCheck className="h-3.5 w-3.5" /> Example proposal
                      </span>
                    </div>

                    <div className="absolute bottom-5 left-5">
                      <span className="inline-flex rounded-full bg-ink/90 px-3.5 py-2 text-[0.65rem] font-bold uppercase tracking-widest text-paper shadow-sm">
                  ...
                  <div className="flex flex-1 flex-col p-7 sm:p-8">
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        <p className="flex items-center gap-1.5 text-[0.65rem] font-bold uppercase tracking-[0.18em] text-coral">
                          <MapPin className="h-3.5 w-3.5" /> {proposal.locality}
                        </p>
                        <h3 className="mt-3 font-display text-3xl font-semibold tracking-[-0.04em] text-ink">{proposal.service}</h3>
                      </div>
                      <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-indigo/5 text-indigo transition-colors duration-500 group-hover:bg-indigo group-hover:text-paper">
                        <ShieldCheck className="h-5 w-5" />
                      </span>
                    </div>

                    <p className="mt-5 text-sm leading-relaxed text-ink/70">{proposal.summary}</p>
                    
                    <div className="mt-auto grid gap-2.5 pt-8">
                      {proposal.evidence.map((item) => (
                        <span key={item} className="flex items-center gap-3 rounded-2xl bg-cream/50 px-4 py-3 text-xs font-semibold text-ink/80 transition-colors group-hover:bg-cream">
          ...
          <p className="mt-5 text-center text-xs text-ink/80">
      ...
      <section className="pb-12 sm:pb-16" aria-labelledby="care-report-title">
        <div className="container-shell">
          <div className="relative overflow-hidden rounded-[3.5rem] border border-indigo/10 bg-[#2f2032] p-6 text-paper shadow-soft sm:p-10 lg:p-14">
            ...
            <div className="relative grid gap-12 lg:grid-cols-[0.82fr_1.18fr] lg:items-center">
              <ScrollReveal direction="left">
                <div>
                  ...
                  <ScrollTextReveal 
                    ...
                    font={{ fontSize: "inherit", fontWeight: "inherit", lineHeight: "inherit" }} 
                  />
                  <p className="mt-6 max-w-xl text-sm leading-7 text-paper/80">
                  ...
                  <div className="mt-7 flex flex-wrap gap-2">
                    {["Timed milestones", "Private photo updates", "Care notes", "Exception trail"].map((item) => (
                      <span key={item} className="rounded-full border border-paper/10 bg-paper/[0.06] px-3 py-2 text-xs font-semibold text-paper/80">
                        {item}
                      </span>
                    ))}
                  </div>
              ...
              <ScrollReveal direction="right">
                <Scale3D>
                  <div className="relative rounded-[2.35rem] border border-paper/[0.12] bg-paper/[0.07] p-4 sm:p-5">
                    <div className="flex flex-col gap-4 rounded-[1.8rem] bg-paper p-5 text-ink shadow-lifted sm:p-6">
                      <div className="flex items-start justify-between gap-4 border-b border-indigo/10 pb-5">
                        <div>
                          <p className="text-[0.62rem] font-bold uppercase tracking-[0.18em] text-indigo/80">Sample care report · PS-2026</p>
                      ...
                      <div className="grid gap-3">
                        {reportEvents.map(({ time, label, detail, icon: Icon }, index) => (
                          <div key={label} className="relative grid grid-cols-[3.4rem_2.6rem_1fr] items-start gap-3">
                            ...
                            <time className="pt-2 text-[0.65rem] font-bold text-ink/80">{time}</time>
                            ...
                            <div className="rounded-2xl bg-cream/72 px-4 py-3">
                              ...
                              <p className="mt-1 text-xs leading-5 text-ink/80">{detail}</p>
                            </div>
                          </div>
                        ))}
                      </div>
                      ...
                      <div className="grid grid-cols-3 gap-2 border-t border-indigo/10 pt-5 text-center">
                        {[["58 min", "Service"], ["4", "Updates"], ["0", "Exceptions"]].map(([value, label]) => (
                          <div key={label} className="rounded-2xl bg-indigo/[0.05] px-2 py-3">
                            ...
                            <p className="mt-0.5 text-[0.6rem] font-bold uppercase tracking-[0.12em] text-ink/80">{label}</p>
                          </div>
                        ))}
                      </div>
                    </div>

                    <div className="mt-4 flex items-center justify-between gap-3 px-2 text-xs text-paper/80">
