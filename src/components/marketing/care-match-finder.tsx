...
import { ArrowRight, CheckCircle2, ChevronDown, Clock3, MapPin, PawPrint, Search, ShieldCheck } from "lucide-react";
...
export function CareMatchFinder() {
  ...
  return (
    <form
      ...
      action="/book"
      method="get"
      className="relative w-full min-w-0 overflow-hidden rounded-[2rem] border border-white/80 bg-white/95 p-4 sm:p-5 lg:p-6 shadow-[0_16px_50px_rgba(0,0,0,0.16)] backdrop-blur-2xl"
    ...
    >
      ...
      <div className="relative flex min-w-0 items-center justify-between gap-2 sm:gap-3">
        <div className="min-w-0">
          <span className="inline-flex items-center gap-1.5 text-[0.62rem] sm:text-[0.68rem] font-bold uppercase tracking-[0.14em] text-[#d45638] font-outfit">
            <span className="h-1.5 w-1.5 rounded-full bg-emerald-500" />
            Instant Care Match
          </span>
          <h2 className="mt-0.5 truncate font-display text-[1.25rem] sm:text-[1.5rem] font-bold tracking-tight text-[#1e1322]">
            Find trusted local care
          </h2>
        </div>
        <span className="inline-flex shrink-0 items-center gap-1 rounded-full bg-[#eaf4ec] px-2.5 py-1 text-[0.65rem] sm:text-[0.7rem] font-bold text-[#357a4e] font-outfit">
      ...
      <div className="relative mt-3.5 grid grid-cols-2 gap-2.5">
        ...
        <label className="group relative col-span-2 flex min-w-0 flex-col gap-0.5 rounded-xl border border-[#e8e2e9] bg-[#fbf9fa] px-3.5 py-2 transition focus-within:border-indigo/40 focus-within:bg-white focus-within:ring-2 focus-within:ring-indigo/10 hover:border-[#d9cfdc]">
          ...
          <div className="relative flex items-center">
            <select
              ...
              className="w-full appearance-none bg-transparent pr-7 text-[0.84rem] sm:text-[0.9rem] font-bold text-[#231526] focus:outline-none cursor-pointer truncate"
            >
        ...
        <label className="group relative col-span-1 flex min-w-0 flex-col gap-0.5 rounded-xl border border-[#e8e2e9] bg-[#fbf9fa] px-3.5 py-2 transition focus-within:border-indigo/40 focus-within:bg-white focus-within:ring-2 focus-within:ring-indigo/10 hover:border-[#d9cfdc]">
          ...
          <div className="relative flex items-center">
            <select
              ...
              className="w-full appearance-none bg-transparent pr-6 text-[0.84rem] sm:text-[0.9rem] font-bold text-[#231526] focus:outline-none cursor-pointer"
            >
        ...
        <label className="group relative col-span-1 flex min-w-0 flex-col gap-0.5 rounded-xl border border-[#e8e2e9] bg-[#fbf9fa] px-3.5 py-2 transition focus-within:border-indigo/40 focus-within:bg-white focus-within:ring-2 focus-within:ring-indigo/10 hover:border-[#d9cfdc]">
          ...
          <input
            ...
            className="w-full bg-transparent text-[0.84rem] sm:text-[0.9rem] font-bold text-[#231526] placeholder:text-[#938290]/50 placeholder:font-normal focus:outline-none truncate"
      ...
      <div className="relative mt-3 flex flex-wrap items-center gap-1.5" aria-label="Suggested cities">
        <span className="text-[0.62rem] sm:text-[0.66rem] font-bold text-[#716573] uppercase tracking-wider mr-0.5">Popular:</span>
      ...
      <div className="relative mt-3.5 flex items-center justify-between gap-2 sm:gap-3 max-[430px]:flex-col max-[430px]:items-stretch">
        <div className="flex min-w-0 items-center gap-2 sm:gap-2.5">
          ...
          <div className="min-w-0">
            <div className="flex min-w-0 items-center gap-1.5">
              <span className="min-w-0 truncate text-[0.8rem] sm:text-[0.84rem] font-bold text-[#1e1322]">{selectedService.label}</span>
              <span className="rounded-full bg-[#eaf4ec] px-1.5 py-0.5 text-[0.62rem] font-bold text-[#357a4e] shrink-0">
                {selectedService.price}
              </span>
            </div>
            <p className="text-[0.64rem] sm:text-[0.7rem] text-[#6d616f] truncate">{selectedService.note}</p>
          </div>
        </div>
        ...
        <Button
          ...
          size="lg"
          disabled={isPending}
          className="shrink-0 rounded-xl bg-[#231526] hover:bg-[#341d39] px-3.5 sm:px-4 py-2.5 sm:py-3 font-outfit text-xs sm:text-[0.74rem] font-bold text-white shadow-lifted hover:shadow-xl transition-all duration-300 border-transparent whitespace-nowrap max-[430px]:w-full max-[430px]:justify-center"
        >
      ...
      <div className="relative mt-3 flex items-center gap-1.5 rounded-xl bg-[#f2f6f3] px-3 py-1.5 text-[0.65rem] sm:text-[0.7rem] font-medium text-[#415647]">
      ...
      <div className="relative mt-3 pt-2.5 border-t border-[#f0eaf1] flex items-center justify-between gap-2 text-[0.68rem] sm:text-[0.72rem] font-medium text-[#655567]">
        ...
        <span className="hidden min-[480px]:inline-flex items-center gap-1 text-[#655567]">
          <Clock3 className="h-3 w-3 text-[#d97706]" />
          <span>Structured service history</span>
        </span>
        <span className="inline-flex items-center gap-1 text-[#655567]">
          <ShieldCheck className="h-3 w-3 text-[#357a4e]" />
