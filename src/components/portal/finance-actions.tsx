...
export function RefundActions({ id, status }: { id: string; status: string }) {
  ...
  return <div className="mt-4"><label className="text-xs font-bold uppercase tracking-[0.14em] text-ink/80">Finance note<input value={note} onChange={(event) => setNote(event.target.value)} minLength={5} maxLength={500} className="mt-2 w-full rounded-2xl border border-ink/10 bg-cream/35 px-4 py-3 text-sm font-normal normal-case tracking-normal outline-none focus:border-coral" placeholder="Reason and reconciliation context" /></label><div className="mt-3 flex flex-wrap gap-2">{actions.map((action) => <button key={action} type="button" disabled={pending || note.trim().length < 5} onClick={() => transition(action)} className="rounded-full bg-ink px-4 py-2 text-xs font-bold text-paper transition hover:bg-coral disabled:opacity-40">{action === "PROCESSING" ? "Send to Razorpay" : action.toLowerCase().replace(/^./, (letter) => letter.toUpperCase())}</button>)}</div>{error && <p role="alert" className="mt-2 text-xs font-semibold text-coral">{error}</p>}</div>;
}
...
export function PayoutActions({ id, status, reviewReady }: { id: string; status: string; reviewReady: boolean }) {
  ...
  return <div className="mt-4"><label className="text-xs font-bold uppercase tracking-[0.14em] text-ink/80">Transfer reference<input value={providerRef} onChange={(event) => setProviderRef(event.target.value)} maxLength={200} className="mt-2 w-full rounded-2xl border border-ink/10 bg-cream/35 px-4 py-3 text-sm font-normal normal-case tracking-normal outline-none focus:border-coral" placeholder="Required before marking paid" /></label><label className="mt-3 block text-xs font-bold uppercase tracking-[0.14em] text-ink/80">Finance decision note<input value={note} onChange={(event) => setNote(event.target.value)} minLength={5} maxLength={1000} className="mt-2 w-full rounded-2xl border border-ink/10 bg-cream/35 px-4 py-3 text-sm font-normal normal-case tracking-normal outline-none focus:border-coral" placeholder="Required for the audit trail" /></label><div className="mt-3 flex flex-wrap gap-2">{available.map((action) => <button key={action} type="button" disabled={pending || note.trim().length < 5 || (action === "PAID" && providerRef.trim().length < 3)} onClick={() => transition(action)} className="rounded-full bg-ink px-4 py-2 text-xs font-bold text-paper transition hover:bg-coral disabled:opacity-40">{action.toLowerCase().replace(/^./, (letter) => letter.toUpperCase())}</button>)}</div>{error && <p role="alert" className="mt-2 text-xs font-semibold text-coral">{error}</p>}</div>;
}
...
function FinanceInput(props: React.InputHTMLAttributes<HTMLInputElement> & { label: string }) { const { label, ...rest } = props; return <label className="mt-4 block text-xs font-bold uppercase tracking-[0.12em] text-ink/80">{label}<input {...rest} className="mt-2 min-h-11 w-full rounded-xl border border-ink/12 bg-paper px-3 py-2 text-sm font-normal normal-case tracking-normal outline-none focus:border-indigo" /></label>; }
