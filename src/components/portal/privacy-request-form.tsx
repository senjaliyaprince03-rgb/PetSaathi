...
export function PrivacyRequestForm() {
  ...
  return (
    <section className="rounded-[1.75rem] border border-ink/[0.07] bg-paper p-5 shadow-[0_18px_55px_-42px_rgb(var(--ink)/0.35)] sm:p-6">
      ...
      <p className="mt-2 text-sm leading-6 text-ink/80">Recording a request never exports or deletes protected data without identity verification and review.</p>
      <div className="mt-6 grid gap-3 sm:grid-cols-3" role="radiogroup" aria-label="Privacy request type">{requestTypes.map((item) => { const active = type === item.value; const Icon = item.icon; return <button key={item.value} type="button" role="radio" aria-checked={active} onClick={() => setType(item.value)} className={`rounded-[1.35rem] border p-4 text-left transition ${active ? "border-indigo/25 bg-indigo/[0.055] shadow-sm" : "border-ink/[0.07] bg-cream/30 hover:bg-cream/55"}`}><span className={`flex h-9 w-9 items-center justify-center rounded-xl ${active ? "bg-indigo text-paper" : "bg-paper text-ink/80"}`}><Icon className="h-4 w-4" /></span><span className="mt-4 block text-sm font-bold">{item.label}</span><span className="mt-1 block text-xs leading-5 text-ink/80">{item.copy}</span></button>; })}</div>
      ...
      <div className="mt-5 flex flex-wrap items-center gap-4"><Button type="button" variant="accent" onClick={submit} disabled={pending || details.trim().length < 20}>{pending ? <LoaderCircle className="h-4 w-4 animate-spin" /> : <Send className="h-4 w-4" />}Record request</Button><p className="text-xs text-ink/80">Identity review follows submission.</p></div>
