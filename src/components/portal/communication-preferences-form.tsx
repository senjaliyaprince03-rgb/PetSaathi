  ...
  return (
    <section className="rounded-[1.75rem] border border-ink/[0.07] bg-paper p-5 shadow-[0_18px_55px_-42px_rgb(var(--ink)/0.35)] sm:p-6">
      ...
      <div className="mt-6 grid gap-3">
        ...
        <div className="flex items-center justify-between gap-4 rounded-[1.35rem] border border-leaf/15 bg-leaf/[0.06] p-4"><div className="flex items-start gap-3"><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl bg-leaf/10 text-leaf"><ShieldCheck className="h-4 w-4" /></span><div><p className="font-bold">In-app essential updates</p><p className="mt-1 text-xs leading-5 text-ink/80">Always enabled for bookings, safety and account security.</p></div></div><span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-leaf text-paper"><Check className="h-4 w-4" /></span></div>
      </div>
...
function Preference({ icon: Icon, label, description, checked, onChange }: { icon: typeof Mail; label: string; description: string; checked: boolean; onChange: (value: boolean) => void }) {
  return <label className={`group flex cursor-pointer items-center justify-between gap-4 rounded-[1.35rem] border p-4 transition ${checked ? "border-indigo/20 bg-indigo/[0.045]" : "border-ink/[0.07] bg-cream/30 hover:bg-cream/55"}`}><span className="flex items-start gap-3"><span className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl transition ${checked ? "bg-indigo text-paper" : "bg-paper text-ink/80 shadow-sm"}`}><Icon className="h-4 w-4" /></span><span><span className="block font-bold">{label}</span><span className="mt-1 block text-xs leading-5 text-ink/80">{description}</span></span></span><span className={`relative h-7 w-12 shrink-0 rounded-full transition ${checked ? "bg-indigo" : "bg-ink/10"}`}><input type="checkbox" checked={checked} onChange={(event) => onChange(event.target.checked)} className="sr-only" /><span className={`absolute top-1 h-5 w-5 rounded-full bg-paper shadow-sm transition ${checked ? "left-6" : "left-1"}`} /></span></label>;
}
