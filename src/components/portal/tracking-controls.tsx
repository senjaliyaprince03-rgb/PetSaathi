...
export function TrackingControls({ assignmentId }: { assignmentId: string }) {
  ...
  return <div className="mt-4 rounded-2xl border border-paper/15 bg-paper/8 p-4"><p className="text-sm font-semibold">Live service location</p><p className="mt-1 text-xs leading-5 text-paper/80">Shared only with the pet parent and authorised operations during this active service. Web browsers require the screen to remain on during walk tracking to stream GPS points; location automatically expires and purges on the 30-day retention schedule.</p><Button type="button" variant={sharing ? "outline" : "accent"} className={sharing ? "mt-3 border-paper/25 text-paper hover:bg-paper/10" : "mt-3"} onClick={sharing ? stop : start} disabled={pending}>{pending ? <LoaderCircle className="h-4 w-4 animate-spin" /> : sharing ? <MapPinOff className="h-4 w-4" /> : <LocateFixed className="h-4 w-4" />}{sharing ? "Stop sharing" : "Start location sharing"}</Button>{error && <p role="alert" className="mt-2 text-xs font-semibold text-saffron">{error}</p>}</div>;
}
