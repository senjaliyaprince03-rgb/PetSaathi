...
- `/api/health` is the availability probe. Alert after 3 consecutive failures over 5 minutes.
...
- Production trace sampling is 10% server-side and 5% client/edge; session replay is intentionally disabled on sensitive care surfaces.
...
6. Medium: p95 server transaction duration above 2 seconds for 15 minutes.
